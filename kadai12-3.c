...
typedef struct {
  ...
  char startname[MAX_NAME_LEN];//名前
}ranking_data_t;
...
int update_ranking(double score,int c,ranking_data_t r[]){
  for(int i= RANKING_SIZE-1;i>=c;i--){
    r[i+1].ranking_score = r[i].ranking_score;//ランキングの最下位に一つ上のランキングのスコアを代入する
    strcpy(r[i+1].startname,r[i].startname);
  }
  r[c].ranking_score = score;//スコアを前の順位に代入
  strcpy(r[c].startname,playername);//自分の順位t→ny→c
}
...
//3ターンの回答入力（ゲーム終了の質問で０と回答するとループ終了）
  ...
  int ny = get_ranking((double)sum/3,r);//②スコアの順位報告

  int nz = update_ranking((double)sum/3,ny,r);//③スコアの入れ替え

  int nw =  print_ranking(r);//ランキングの表示

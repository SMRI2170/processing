...
//ランキングデータを読み込む関数int load_ranking()
...
//3ターンの回答入力（ゲーム終了の質問で０と回答するとループ終了）
  ...
  FILE *fp;
   ...
   if(fp==NULL){
    ...
    fread(r,sizeof(ranking_data_t),sizeof(r)/sizeof(r[0]),fp);
  
    fclose(fp);
   }

   int np = print_ranking(r);
  ...
  if(fp==NULL){
    ...
    fwrite(r,sizeof(ranking_data_t),sizeof(r)/sizeof(r[0]),fp);
    fclose(fp);
  }
